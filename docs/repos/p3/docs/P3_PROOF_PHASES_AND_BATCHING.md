...
For ordinary shared review, capture remains independent of cloud availability by staging the write-once run under `C:\P3Proofs` first. Durable publication hash-verifies the original PNG/video under `G:\Oma Drive\P3 Visual Evidence\p3` and records deterministic Google Drive / ChatGPT Library identity for later retrieval. When the exact media is already visible or directly inspectable in the current model/session, review it there with the native image/video surface. After session expiry or when the exact media is otherwise unavailable, reopen that same durable original through the connected Google Drive -> ChatGPT Library source identity; do not re-upload a local copy merely because the session changed. MCPVisual exact-file upload is only for local-only media that has no model-openable durable identity. Do not use ZIP materialization, PowerShell/base64, OCR, custom HTTP, Git/LFS, re-encoding, or another proof store merely to expose pixels.
