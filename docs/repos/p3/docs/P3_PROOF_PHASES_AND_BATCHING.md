...
For ordinary shared review, capture remains independent of cloud availability by staging the write-once run under `C:\P3Proofs` first. Durable publication may copy and hash-verify the original PNG/video under `G:\Oma Drive\P3 Visual Evidence\p3`, but publication storage does **not** dictate the review tool. When the exact media is already visible or directly inspectable in the current model/session, review it there with the native image/video surface. Do not bounce proof through MCP media payloads, PowerShell/base64, OCR, custom HTTP, Drive/Library, Git/LFS, or another store merely to expose pixels. Use retrieval only when the exact media is otherwise unavailable.
