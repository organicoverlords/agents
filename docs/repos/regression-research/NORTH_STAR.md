...
## Finished assistant-stack roadmap

The finished assistant stack is not a collection of clever control-plane components. It is a **self-maintaining, boring operating system for useful agent work**: the user states direction, workers recover the right live authority, coordinate exact collisions, keep work converging, survive ordinary failures and return evidence without requiring the user to operate the machinery.

The durable progression is:

1. **One authority model.** Shared rules, project North Stars, issues/PRs, live Git/runtime evidence and Busy ownership have non-overlapping roles; stale history/dashboards never become accidental authority.
2. **Reliable execution and recovery.** MCP, Busy, Git/GitHub, build/proof routes and plugins fail locally, recover through explicit bounded routes and preserve unique/foreign work.
3. **Autonomous swarm convergence.** Workers reconcile existing WIP before fan-out, maintain swarm-friendly parent/child issues and Project projections, recover an exhausted issue wall themselves, and move from implementation into proof/integration when live downstream pressure demands it. No orchestrator chat is a permission gate.
4. **Useful evidence and learning.** Vault history, reports, fixtures and regression research make known failures expensive to repeat without becoming startup truth, liveness state or a second scheduler.
5. **Finished boring stack.** A user can change direction in any worker, leave the machine unattended, and expect the swarm to keep pursuing the current product North Stars, repairing work representation and infrastructure when necessary, converging existing work and escalating only real user decisions.

The user is responsible for product intent and genuine external decisions—not issue gardening, worker supervision, routine recovery, stale-state cleanup or choosing which internal subsystem should act next.
