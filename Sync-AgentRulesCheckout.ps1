...
function Invoke-GitText {
    ...
    $errFile = Join-Path ([IO.Path]::GetTempPath()) ("agents-sync-giterr-" + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    if ($indexTree -ne $headTree) {
        $indexCommit = Git-Text @('commit-tree',$indexTree,'-p',$headBefore,'-m','Preserve live .agents index before canonical convergence')
    ...
    if ($worktreeTree -ne $indexTree -or $null -eq $indexCommit) {
        $preserveCommit = Git-Text @('commit-tree',$worktreeTree,'-p',$preserveParent,'-m','Preserve live .agents worktree before canonical convergence')
    }
    $stamp = (Get-Date).ToUniversalTime().ToString('yyyyMMdd-HHmmss')
    $suffix = [Guid]::NewGuid().ToString('N').Substring(0,6)
    $preserveBranch = "preserve/agents-live-$stamp-$suffix"
