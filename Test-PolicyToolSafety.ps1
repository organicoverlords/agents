...
foreach ($required in @(
    ...
    'BusyCoordinator is an exact shared-mutation collision lock',
    'A claim blocks only that exact mutation scope',
    'Do not use coordinator metadata to select work, steer peers, reserve capacity, decide liveness, or recycle another actor''s lane',
    'Once the requested failure is fixed and the current live path is verified, stop',
