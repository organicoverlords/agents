...
$required = @(
  ...
  'reopen the persistent originals by durable identity',
  'permission-bearing emergency transport only',
  'never select or retry it automatically as normal visual-review orchestration',
  'backfill those exact originals into the owning durable Library/Files store before review',
...
$requiredRouting = @(
  ...
  'permission-bearing manual emergency transport only after explicit user approval',
  'it is never an automatic review fallback',
...
$forbidden = @('Drive/Library detours','one transient ZIP carrying the exact original media plus a compact manifest','Multi-file payloads use one transient ZIP','transient ZIP+manifest','otherwise transfer the individual originals directly in a bounded sequence of `upload_local_file` calls')
